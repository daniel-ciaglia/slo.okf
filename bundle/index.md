...
A worked example of the [SRE OKF vocabulary](../VOCABULARY.md) applied to one critical customer journey: checkout. This is a pilot, not a finished product catalog.

This directory (`bundle/`) is the actual OKF bundle — every non-reserved `.md` file below is a
concept with `type` frontmatter. `VOCABULARY.md`, `README.md`, and `validator/` are
