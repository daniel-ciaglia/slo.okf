...
output "concept_id" {
  ...
  description = <<-EOT
    ...
    concept_id_for() exactly. Paste directly into e.g. Alert.slo or Service.slos on
    another concept: slo = module.checkout_availability_slo.concept_id.
...
output "relative_path" {
  ...
  description = <<-EOT
    ...
    "../slos/" prefix and use "$${var.id}.md" directly instead.
  EOT
}
...
output "md_link" {
  ...
  description = <<-EOT
    ...
    freetext = "Backs $${module.checkout_availability_slo.md_link}."
  EOT
}
