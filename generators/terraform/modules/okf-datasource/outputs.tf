...
output "concept_id" {
  ...
  description = <<-EOT
    ...
    concept_id_for() exactly. Paste directly into e.g. Metric.data_source or
    SLI.data_source on another concept: data_source = module.prometheus_datasource.concept_id.
...
output "relative_path" {
  ...
  description = <<-EOT
    ...
    "../datasources/" prefix and use "$${var.id}.md" directly instead.
  EOT
}
...
output "md_link" {
  ...
  description = <<-EOT
    ...
    freetext = "Backs every metric referenced by $${module.prometheus_datasource.md_link}."
  EOT
}
