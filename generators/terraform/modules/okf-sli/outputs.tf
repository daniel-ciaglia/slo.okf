...
output "concept_id" {
  ...
  description = <<-EOT
    ...
    concept_id_for() exactly. Paste directly into e.g. SLO.sli on another concept:
    sli = module.checkout_availability_sli.concept_id.
...
output "relative_path" {
  ...
  description = <<-EOT
    ...
    "../slis/" prefix and use "$${var.id}.md" directly instead.
  EOT
}
...
output "md_link" {
  ...
  description = <<-EOT
    ...
    freetext = "Feeds the $${module.checkout_availability_sli.md_link}."
  EOT
}
