...
**Migrated to VOCABULARY.md v0.2.0** (see its Changelog): `okf-subsystem`'s `journeys` input is
gone and `service` is now required, not optional; `okf-service` gained `subsystems`/`journeys`
(informational back-refs) and `parent`/`children`, plus an optional `resource`, since `Service`
is no longer pure SLO grouping. `../examples/cart-service/` and `../examples/checkout-service/`
(the old `Subsystem`-typed `cart-service`/`payment-service` pair, and the synthetic
grouping-only `Service` example) are retired, replaced by `../examples/checkout-services/`
(now mirrors the real, migrated `bundle/services/{cart,payment}-service.md`),
`../examples/cart-service-redis/` (the one real `Subsystem` left in `bundle/` post-migration),
and `../examples/checkout-platform/` (synthetic — demonstrates `parent`/`children`, which no
real `bundle/services/*.md` file uses yet).
...
- Required vocabulary fields (`title`, `resource`) plus optional ones (`description`, `owner`,
  `tags`, `services`, `journeys`) as typed Terraform variables, enforced with `validation`
...
human calling the module — never inferred from anything: `okf-subsystem`'s `services`,
`okf-service`'s `slos`/`subsystems`/`journeys`/`parent`/`children`, `okf-metric`'s
`data_source`, `okf-sli`'s `ratio_metric`/`threshold_metric`/`data_source`, `okf-slo`'s
`sli`/`journey`, `okf-alert`'s `slo`/`runbook`. Same rule as the original design's "never
invents CustomerJourney or SLO links," just generalized: instead of a generator abstaining and
leaving a concept unreferenced, the human who's already at the call site states the link
directly. Back-ref fields (§3's "informational" column — `Service.subsystems`,
`Service.journeys`, `Service.children`, `SLO.journey`) still need their forward-side
counterpart hand-authored to satisfy rule 9's symmetry check; no module writes both sides of a
link.
...
- **Real adoption**: nothing in the actual `bundle/` is generated by this yet; every file these
  modules reproduce (`services/cart-service.md`, `datasources/prometheus-prod.md`, etc.) stays
  hand-authored there until this is deliberately adopted for it.
